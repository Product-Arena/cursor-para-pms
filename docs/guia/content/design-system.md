...
- Pasta modelo de workspace — referência `workspace-cursor-modelo.md` (sem slide de “copiar curso para a raiz”)
...
- **Slides de exercícios e demos escuras**  
  ...
  - Uso: todos os exercícios (Ask, Plan, Agent, Plan → Build, Ask com contexto, MCP Notion) e a demo principal (“Seu dia pode começar com um comando”).
...
### Aplicar a
- Todos os slides com classe `slide-type-exercicio`: Ask, Plan (mapear e clonar), Ask com contexto, Agent, MCP Notion. Manter sempre: label canto esquerdo, título centralizado, descrição à esquerda, imagem ou GIF à direita (ou placeholder até ter asset).
...
- **Referência de proporção/espacamento:** **slide Allowlist** e GitHub (sem SSH).  
...
## 🎨 Design Systems para AI Coding

### O que é um DESIGN.md?

Um `DESIGN.md` é um documento que define o design system de um produto de forma que agentes de IA possam aplicá-lo consistentemente ao gerar código. É a ponte entre a identidade visual e o código gerado.

### Por que isso importa para PMs?

| Sem Design System | Com Design System |
|-------------------|-------------------|
| ❌ Cada tela tem cores diferentes | ✅ Paleta consistente em todo app |
| ❌ Botões mudam de estilo | ✅ Componentes padronizados |
| ❌ "Deixa bonitinho" = retrabalho | ✅ Especificação clara para o agente |
| ❌ Design debt desde o MVP | ✅ Base sólida para escalar |

### Recursos recomendados

#### 🔗 [getdesign.md](https://getdesign.md)

Coleção open source de DESIGN.md prontos para usar, inspirados em empresas como:

- **Vercel** — Preto e branco, precisão, fonte Geist
- **Stripe** — Gradientes roxos, elegância peso-300
- **Linear** — Ultra-minimal, preciso, accent roxo
- **Notion** — Minimalismo quente, serifas, superfícies suaves
- **Supabase** — Tema dark esmeralda, code-first

**Como usar:**
1. Acesse [getdesign.md](https://getdesign.md)
2. Encontre um design system que combine com seu produto
3. Copie o arquivo DESIGN.md para seu projeto
4. O Cursor vai aplicar o estilo automaticamente

#### 📦 Design System da Arena

Para este curso, usamos o design system da Product Arena (este documento). Ao criar dashboards e visualizações, aplique:

- **Accent:** `#FF5757` (Product Arena Red)
- **Backgrounds:** `#f5f5f5` → `#ebebeb` → `#e0e0e0`
- **Tipografia:** Inter para texto, SF Mono para código
- **Estilo:** Notion-like, limpo, emojis como ícones

### Aplicação prática

Quando pedir ao Cursor para criar uma visualização ou dashboard:

```markdown
# Criar dashboard de métricas

## Design system
Seguir o design system da Arena:
- Cor accent: #FF5757
- Background: #f5f5f5
- Tipografia: Inter
- Estilo: Notion-like, cards com sombra suave
...
- **getdesign.md:** https://getdesign.md — Coleção de DESIGN.md para AI coding
